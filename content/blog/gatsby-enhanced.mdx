...
I recently [decided to start a blog](https://ph-fritsche.github.io) which raised the question what platform to use for this venture. And what kind of developer would I be if I didn't see this as a golden opportunity to learn some new things along the way. I pondered trying out [Gatsby](https://www.gatsbyjs.com/) for a while now and this was my chance to experience all the gloriousness and dreadfulness of just another ecosystem first hand.
...
See the entry for [GatsbyNode.onCreatePage](https://www.gatsbyjs.com/docs/reference/config-files/gatsby-node/#onCreatePage).\
...
So the great news is: Beside the [support for Typescript](https://www.gatsbyjs.com/docs/how-to/custom-configuration/typescript/) for page components there is also a way to tranform (almost) all API hooks into Typescript.
The cleanest ans easiest way I found was per[`gatsby-plugin-ts-config`](https://www.npmjs.com/package/gatsby-plugin-ts-config).
It allows to reduce the contents of `gatsby-config.js` in the project root to a bare minimum and select another directory for the rest of the configuration and writing them in Typescript.
...
One limitation I could not (cleanly) work around was the support for `compilerOptions.paths`. You can use [`ts-node`](https://www.npmjs.com/package/ts-node) and pass options to it.
